...
function UploadItem({ userID }: UploadItemProps) {
  ...
  return (
    <>
      {modalVisible && (
        <div className="modal fixed top-0 left-0 right-0 bottom-0 flex justify-center items-center bg-black opacity-70 z-20">
          <div className="bg-white p-20 flex flex-col gap-5">
            <input className='border border-black p-2 rounded-2xl' type="text" value={imageUrl} onChange={handleImageChange} />
            <button onClick={() => handleCloseModal(imageUrl)}>Close</button>
          </div>
        </div>
      )}
